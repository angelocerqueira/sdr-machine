...
export function PipeMini({ current, className = "", onStageClick }: PipeMiniProps) {
    ...
    <div className={`flex items-center gap-0 ${className}`}>
      {STAGES.map((stage, i) => {
        ...
        const dot = isPast ? (
          ...
          >
            <div className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--text-strong)" }} />
          </div>
        ) : (
          <div
        ...
        return (
          <div key={stage.key} className="flex items-center">
            {i > 0 && (
              <div className="w-4 h-px mx-0.5" style={{ background: "var(--line-1)" }} />
            )}
            <div className="relative group">
              {interactive ? (
                <button
                  ...
                  disabled={isCurrent}
                  aria-label={`Mover para ${stage.label}`}
                  className={`block p-0 bg-transparent border-0 ${isCurrent ? "cursor-default" : "cursor-pointer hover:scale-110"} transition-transform`}
                >
                  {dot}
                </button>
              ) : (
